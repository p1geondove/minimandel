...
Vadd = lambda a,b : (a[0]+b[0],a[1]+b[1])   #add/subtract/multiply/divide 2 tuples and/or lists like numpy arrays
...
def hanlde_input(event, mpressed, viewoffset, scale, res):
    if event.type == pygame.QUIT:   #Exit pressing close button of the window
        pygame.quit()
        sys.exit()
    
    elif event.type == KEYDOWN:
        if event.key == K_ESCAPE:   #exit using 'Esc' button
            pygame.quit()
            sys.exit()
        ...
        if event.button == 1: #left mousebutton
            ...
            viewoffset = Vmul(viewoffset,(2,2)) #double the zoom
            viewoffset = Vsub(viewoffset,mpos)  #go middle to the mouse
            scale *= 2
        if event.button == 5: #zoom out
            viewoffset = Vadd(viewoffset,mpos)  #go middle to the mouse
            viewoffset = Vdiv(viewoffset,(2,2)) #half the zoom
            scale /= 2
...
def run():
    ...
    scale = 50  #the zoom level, 1 unit is {scale} pixel
    winsize = (300,300)
    ...
    while True:
        for y in range(0,winsize[1],res):       #for every virtual pixel (resolution, not screen pixel)
            for x in range(0,winsize[0],res):
                pixelpos = complex(Vdiv(Vsub((x,y),viewoffset),[scale]*2))  #get complex value of pixel     
